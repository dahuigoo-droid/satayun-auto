...
from saju_calculator import (
    ...
    calc_합충형파해, calc_천간합, calc_궁성, calc_육친, 
    calc_납음오행, calc_격국, calc_공망_전체, calc_용신
)
...
천간십성 {천간십성['시']}     (일간)      {천간십성['월']}     {천간십성['년']}
지지십성 {지지십성['시']}     {지지십성['일']}     {지지십성['월']}     {지지십성['년']}
12운성  {운성['시']}        {운성['일']}        {운성['월']}        {운성['년']}
...
"""
    ...
    존재하는 = [(k, v) for k, v in sorted_오행 if v > 0]
    약한 = 존재하는[-1][0] if 존재하는 else "-"
    없는 = [k for k, v in 오행.items() if v == 0]
...
■ 6. 십성표
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
천간십성: 년({천간십성['년']}) 월({천간십성['월']}) 일(일간) 시({천간십성['시']})
지지십성: 년({지지십성['년']}) 월({지지십성['월']}) 일({지지십성['일']}) 시({지지십성['시']})
...
■ 8. 12운성표
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
일간({일주[0]}) 기준 각 지지별 12운성:
년지({년주[1]}): {운성['년']} | 월지({월주[1]}): {운성['월']} | 일지({일주[1]}): {운성['일']} | 시지({시주[1]}): {운성['시']}
...
"""
    ...
    if 합충형파해['삼합']:
        for 항목 in 합충형파해['삼합']:
            if isinstance(항목, dict):
                text += f"- {항목.get('지지', '')} → {항목.get('오행', '')}{'(완성)' if 항목.get('완성') else '(미완성)'}\n"
            else:
                text += f"- {항목}\n"
    else:
        text += "- 없음\n"
    ...
    if 합충형파해['방합']:
        for 항목 in 합충형파해['방합']:
            if isinstance(항목, dict):
                지지_str = ','.join(항목.get('지지', []))
                text += f"- {지지_str} → {항목.get('오행', '')}{'(완성)' if 항목.get('완성') else '(미완성)'}\n"
            else:
                text += f"- {항목}\n"
    else:
        text += "- 없음\n"
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
"""
    for 주, 데이터 in 육친.items():
        천간_육친 = 데이터['천간']['육친']
        지지_육친 = 데이터['지지']['육친']
        text += f"{주}주: 천간({천간_육친}), 지지({지지_육친})\n"
    
    text += f"""
■ 13. 납음오행표
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
년주({년주[0]}{년주[1]}): {납음['년']['납음']} - {납음['년']['설명']}
월주({월주[0]}{월주[1]}): {납음['월']['납음']} - {납음['월']['설명']}
일주({일주[0]}{일주[1]}): {납음['일']['납음']} - {납음['일']['설명']}
시주({시주[0]}{시주[1]}): {납음['시']['납음']} - {납음['시']['설명']}

■ 14. 격국표
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
정격: {격국['정격']}
특수격: {', '.join(격국['특수격']) if 격국['특수격'] else '없음'}
월지: {격국['월지']} (본기: {격국['월지_본기']})

■ 15. 공망표
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
년주 기준 공망: {공망['년']['공망'][0]}, {공망['년']['공망'][1]} ({공망['년']['순']})
일주 기준 공망: {공망['일']['공망'][0]}, {공망['일']['공망'][1]} ({공망['일']['순']})
원국 내 공망 해당: {', '.join(공망['공망_해당']) if 공망['공망_해당'] else '없음'}
...
[조후용신] {용신_data['조후_용신']}
[억부용신] {용신_data['억부_용신']}
[통관용신] {용신_data['통관_용신'] if 용신_data['통관_용신'] else '없음'}
...
with tab1:
    ...
    with btn_col2:
        GPT_버튼 = st.button("📋 GPT용 텍스트", type="secondary", use_container_width=True)
    ...
    if 'gpt_text' in st.session_state:
        ...
        st.download_button(
            ...
            file_name=f"{st.session_state.gpt_이름}_사주데이터.txt",
            mime="text/plain",
    ...
    if 생성_버튼:
        if not 이름:
            ...
            with st.spinner("이미지 생성 중..."):
                ...
                if '16_용신표' in 생성된_이미지:
                    st.subheader("🎯 용신표")
                    st.image(생성된_이미지['16_용신표'], caption=f"{이름}님 용신표")
        ...
        if st.button("🎯 일괄 생성", type="primary", use_container_width=True):
            ...
            with zipfile.ZipFile(zip_buffer, 'w', zipfile.ZIP_DEFLATED) as zf:
                for idx, row in df.iterrows():
                    ...
                    if 용신표_체크:
                        path = f"/tmp/{row['이름']}_용신표.png"
                        create_용신표(사주, 기본정보, path)
                        zf.write(path, f"{folder_name}/16_용신표.png")
